...
動画向けにサムネグリッドのサイズを横長にも出来ます（テーマライト）
...
本アプリを初めて起動した際、Windowsから以下のような
確認メッセージが表示されることがあります。

  ・「Windows によって PC が保護されました」
    （Microsoft Defender SmartScreen の警告）

これは、本アプリが個人で作成した実行ファイル（exe）で
あり、Microsoftなどの第三者による署名がされていない
ために表示されるものです。悪意のあるプログラムでは
ありませんので、内容をご確認のうえ、下記の操作で
許可してください。

  ・SmartScreenの警告が出た場合
    → 「詳細情報」をクリック →「実行」を選択

  ・セキュリティソフト等からアクセス許可を求められた場合
    → 「許可する」「アクセスを許可」を選択
